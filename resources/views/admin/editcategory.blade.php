      ...
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-body add-post">
            <form class="row needs-validation " novalidate="" method="post" action="{{route('admin.category.update', ["id"=>$category->id])}}" enctype='multipart/form-data'>
                @csrf
                @method('PUT')
              <div class="col-sm-12">
                <div class="mb-3">
                  <label for="validationCustom01">Title</label>
                  <input class="form-control" id="validationCustom01" type="text" name="name" placeholder="Add Category Title" value="{{$category->name}}" required>
                </div>
                <div class="mb-3">
                  <label for="validationCustom03">Title in kannada</label>
                  <input class="form-control" id="validationCustom03" type="text" name="name_ka" placeholder="please enter tag name" value="{{$category->name_ka}}" required="">
                </div>
                ...
                <div class="mb-3">
                    <figure class="col-md-3 col-6 img-hover hover-1" itemprop="associatedMedia" itemscope="">
                        <a href="{{asset('uploads/category')}}/<?=$category->image;?>" itemprop="contentUrl" data-size="1600x950" target="_blank">
                          <div>
                              <img src="{{asset('uploads/category')}}/<?=$category->image;?>" itemprop="thumbnail" alt="Image description" class="img-thumbnail" width="150" height="90">
                              </div>
                        </a>
                        <figcaption itemprop="caption description"><?=$category->image;?></figcaption>
                    </figure>
