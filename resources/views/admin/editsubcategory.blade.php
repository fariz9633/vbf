      ...
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-body add-post">
            <form class="row needs-validation " novalidate="" method="post" action="{{route('admin.subcategory.update', ["id"=>$subcategory['det']->id])}}" enctype='multipart/form-data'>
                @csrf
                @method('PUT')
              <div class="col-sm-12">
                <div class="mb-3">
                  <label for="validationCustom01">Title</label>
                  <input class="form-control" id="validationCustom01" type="text" name="name" placeholder="Add Sub category Title" value="{{$subcategory['det']->name}}" required>
                </div>
                <div class="mb-3">
                  <label for="validationCustom03">Title in kannada</label>
                  <input class="form-control" id="validationCustom03" type="text" name="name_ka" placeholder="please enter tag name" value="{{$subcategory['det']->name_ka}}" required="">
                </div>
                <div class="mb-3">
                          <label class="form-label" for="validationCustom04">Category</label>
                          <select class="form-select" id="validationCustom04" name="cat_id" required="" >
                            <option selected disabled="" value="">Choose any one</option>
                            @foreach($subcategory['category'] as $cat)
                             @php 
                             $categorysel = ($cat->id == $subcategory['det']->cat_id)?"selected":"";
                             @endphp
                ...
                <div class="mb-3">
                    <figure class="col-md-3 col-6 img-hover hover-1" itemprop="associatedMedia" itemscope="">
                        <a href="{{asset('uploads/subcategory')}}/<?=$subcategory['det']->image;?>" itemprop="contentUrl" data-size="1600x950" target="_blank">
                          <div>
                              <img src="{{asset('uploads/subcategory')}}/<?=$subcategory['det']->image;?>" itemprop="thumbnail" alt="Image description" class="img-thumbnail" width="150" height="90">
                              </div>
                        </a>
                        <figcaption itemprop="caption description"><?=$subcategory['det']->image;?></figcaption>
                    </figure>
                ...
                <div class="mb-3">
                  <div class="media">
                    ...
                    <div class="media-body text-end">
                      <label class="switch">
                          @php
                          $chk = $subcategory['det']->status == '1' ? "checked" : " ";
                          @endphp
