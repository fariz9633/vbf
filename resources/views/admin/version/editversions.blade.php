      ...
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-body add-post">
            ...
            <form class="f1" method="post" action="{{route('admin.versions.update', ["id"=>$versions->id])}}" enctype='multipart/form-data'>
                 @csrf
                  @method('PUT')
                  @php
                  $cntrow = count(explode('||',$versions->title));
                  ...
                  <input class="form-control" id="versions" type="number" name="versions" placeholder="Add Version number" value="{{$versions->versions}}" required="">
                </div>
                <div class="append">
                    
                    
                     @php
              $t = explode('||',$versions->title);
              $p = explode('||', $versions->descp);
              $im = explode('||', $versions->image);
              $k=1;
              @endphp
              ...
              <div class="mb-3">
                ...
                <div class="mb-3">
                    <figure class="col-md-3 col-6 img-hover hover-1" itemprop="associatedMedia" itemscope="">
                        <a href="{{asset('uploads/versions')}}/{{$im[$key]}}" itemprop="contentUrl" data-size="1600x950" target="_blank">
                          <div>
                              <img src="{{asset('uploads/versions')}}/{{$im[$key]}}" itemprop="thumbnail" alt="Image description" class="img-thumbnail" width="150" height="90">
                              </div>
                        </a>
