      ...
      <div class="col-sm-12">
        <div class="card">
          ...
          <div class="card-body add-post">
            <form class="row needs-validation " novalidate="" method="post" action="{{route('admin.state.update', ["id"=>$state['det']->id])}}" enctype='multipart/form-data'>
                @csrf
                @method('PUT')
              <div class="col-sm-12">
                <div class="mb-3">
                  <label for="validationCustom01">Title</label>
                  <input class="form-control" id="validationCustom01" type="text" name="name" placeholder="Add State Title" value="{{$state['det']->name}}" required>
                </div>
                <div class="mb-3">
                  <label for="validationCustom03">Title in kannada</label>
                  <input class="form-control" id="validationCustom03" type="text" name="name_ka" placeholder="please enter tag name" value="{{$state['det']->name_ka}}" required="">
                </div>
                <div class="mb-3">
                          <label class="form-label" for="validationCustom04">Category</label>
                          <select class="form-select" id="validationCustom04" name="cou_id" required="" >
                            <option selected disabled="" value="">Choose any one</option>
                            @foreach($state['country'] as $cat)
                             @php 
                             $countrysel = ($cat->id == $state['det']->cou_id)?"selected":"";
                             @endphp
                            <option value="{{$cat->id}}" <?=$countrysel;?>>{{$cat->name}}</option>
                            @endforeach
                          </select>
                ...
                <div class="mb-3">
                    <figure class="col-md-3 col-6 img-hover hover-1" itemprop="associatedMedia" itemscope="">
                        <a href="{{asset('uploads/state')}}/<?=$state['det']->image;?>" itemprop="contentUrl" data-size="1600x950" target="_blank">
                          <div>
                              <img src="{{asset('uploads/state')}}/<?=$state['det']->image;?>" itemprop="thumbnail" alt="Image description" class="img-thumbnail" width="150" height="90">
                              </div>
                        </a>
                        <figcaption itemprop="caption description"><?=$state['det']->image;?></figcaption>
                    </figure>
                ...
                <div class="mb-3">
                  <div class="media">
                    ...
                    <div class="media-body text-end">
                      <label class="switch">
                          @php
                          $chk = $state['det']->status == '1' ? "checked" : " ";
                          @endphp
